...
- NormSim 수식:
    ...
    - $X_{taget}=\{x_{target,(1)},\ldots,x_{target,(m)}\}$, where $x_{taget,(i)} \in \real^{d}, i.i.d.-sampled$ from 타겟 downstream task 분포
...
$\text{argmax}_{S_{i+1}\subset S_{i}} \sum_{x \in S}\text{NormSim}_2(S_i, x)$
